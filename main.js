...
/*
  const materialll = new THREE.LineBasicMaterial( { color: 0x0000ff } );


  const pointsss = [];
  pointsss.push( new THREE.Vector3( - 10, 0, 0 ) );
  pointsss.push( new THREE.Vector3( 0, 10, 0 ) );
  pointsss.push( new THREE.Vector3( 10, 0, 0 ) );

  const geometryyy = new THREE.BufferGeometry().setFromPoints( pointsss );

  const lineee = new THREE.Line( geometryyy, materialll );
  scene.add( lineee );
*/
